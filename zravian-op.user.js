...
// @version      4.1
// @description  Multi-account, multi-villaggio, stats, AI farm (SAFE, corretta)
// @match        *://zravian.com/*
...
(function () {
    ...
    const getPlayer = () => $('.playerName')?.textContent?.trim() || 'default';
    const getVillage = () => $('#villageName')?.textContent?.trim() || 'village';
    ...
    function available() {
        const m = {};
        $$('input[name][max]').forEach(i => { if (i?.name) m[i.name] = +i.max || 0; });
        return m;
    }

    function canSend(t) {
        const a = available();
        return Object.entries(t.troops).every(([k, v]) => (a[k] || 0) >= v);
    }

    function cdLeft(t) {
        return Math.max(0, (t.cooldown * 60000) - (now() - (state.last[t.name] || 0)));
    }

    // semplice "AI" per scegliere il farm migliore
    function score(t) {
    ...
    function bestTemplate() {
        const candidates = TEMPLATES.map((t, i) => ({ i, s: score(t) }))
            .sort((a, b) => b.s - a.s);
    ...
    function fill(i) {
        const t = TEMPLATES[i];
        if (!t) return;
        Object.entries(t.troops).forEach(([k, v]) => { const inp = $(`input[name="${k}"]`); if (inp) inp.value = v; });
        const x = $('input[name="x"]'); if (x) x.value = t.x;
        const y = $('input[name="y"]'); if (y) y.value = t.y;
